...
1. Сохраняет две исходные jobs и все базовые steps.
2. Собирает и публикует оба образа.
3. Проверяет Compose-конфигурацию.
4. Сканирует локальные образы Trivy с блокировкой по `HIGH` и `CRITICAL`.
5. Выполняет smoke-тесты запущенного приложения.
